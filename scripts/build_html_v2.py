...
THEMES = {
    "日式": dict(key="jp", flag="🇯🇵", accent="#B5651D", accent2="#C0392B", bg="#F7F3EC",
                 ...
                 radius="10px", vertical=True, tagline="和の静けさ · 侘寂留白"),
    "法式": dict(key="fr", flag="🇫🇷", accent="#C9A227", accent2="#7B2D3A", bg="#FBF6EA",
                 ...
                 radius="14px", vertical=False, tagline="宫廷优雅 · 甜点如艺"),
    "北欧": dict(key="dk", flag="🇩🇰", accent="#6B7A8F", accent2="#9AA7B4", bg="#F4F6F8",
                 ...
                 radius="4px", vertical=False, tagline="极简克制 · 功能主义"),
...
for idx, key in enumerate(FLAGSHIPS, 1):
    ...
    # 加映：同风格其它店，有图则带图小卡，无图则文字卡
    others = [x for x in data if x["cat"] == c["cat"] and x["key"] != key][:4]
    more_cards = ""
    for o in others:
        oimgs = load_imgs(o["key"], 1)
        cover = f'<img class="mini-cover" src="{oimgs[0]}" alt="{o["name"]}" loading="lazy">' if oimgs else ""
        more_cards += f'''
        <div class="more-card {'has-img' if oimgs else 'no-img'}">
          {cover}
          <div class="more-body">
...
.shop{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);
...
.more-title{font-family:var(--font-head);font-size:18px;color:var(--muted);margin-bottom:14px;}
.more-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px;}
.more-card{display:flex;gap:12px;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);padding:12px;font-size:14px;color:var(--ink);}
.more-card.has-img .mini-cover{width:84px;height:112px;object-fit:cover;border-radius:calc(var(--radius) - 4px);flex:0 0 auto;border:1px solid var(--line);}
.more-body{flex:1;min-width:0;}
.more-head{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px;}
.more-head b{font-size:15px;font-family:var(--font-head);}
.more-card p{margin:3px 0;color:var(--ink);font-size:13px;}
.more-card .mk{display:inline-block;color:var(--accent);font-weight:700;margin-right:6px;font-size:12px;}
...
<header class="hero">
  ...
  <p class="sub">按美学风格挑一家对味的店——日式留白、法式甜点、北欧极简、韩系公主感……八个国家，八种气质。</p>
  <div class="badges">
...
<main id="top">
  <section class="intro">
    ...
    <p>每家都给了：<b>出片点</b>（为什么好拍）、<b>招牌推荐</b>（点什么不踩雷）、<b>位置交通</b>、<b>价位</b>、<b>营业时间</b>，还配了<b>实拍图</b>。图来自小红书 / Instagram 真实探店笔记，仅供风格参考。</p>
...
<footer>
  ...
  <p>全 40 家「各国风」清单已结构化存档；本版旗舰 + 同风格加映均含实拍图，可随时解锁完整版。</p>
